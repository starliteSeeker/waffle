...
impl TilemapEditor {
    pub fn handle_action(&self, state: &Window) {
        ...
        imp.flip_x_btn.connect_active_notify(clone!(
            #[weak]
            imp,
            move |btn| {
        ...
        imp.flip_y_btn.connect_active_notify(clone!(
            #[weak]
            imp,
            move |btn| {
        ...
        imp.priority_btn.connect_active_notify(clone!(
            #[weak]
            imp,
            move |btn| {
        ...
        state.connect_tileset_sel_idx_notify(clone!(
            ...
            move |state| {
                if let Err(e) = imp
                    .curr_tile
                    .borrow_mut()
                    .set_tile_idx_checked(state.tileset_sel_idx() as u16)
                {
                    eprintln!("{e}");
                };
            }
        ...
        imp.zoom_select.connect_selected_notify(clone!(
            #[weak(rename_to = this)]
            self,
            move |_| {
                let imp = this.imp();
                let new_zoom = Zoom::iter()
                    .nth(imp.zoom_select.selected() as usize)
                    .expect("shouldn't happen");
                this.set_tilemap_zoom(new_zoom);
            }
        ));
        imp.mode_select.connect_selected_notify(clone!(
            #[weak]
            state,
            #[weak]
            imp,
            move |_| {
                state.set_bg_mode(
                    BGModeTwo::iter()
                        .nth(imp.mode_select.selected() as usize)
                        .expect("shouldn't happen"),
                );
            }
        ));
    ...
    pub fn render_widget(&self, state: &Window) {
        self.connect_tilemap_zoom_notify(clone!(
            #[weak(rename_to = this)]
            self,
            move |_| {
                let imp = this.imp();
        ...
        state.connect_tilemap_data_notify(clone!(
            #[weak(rename_to = this)]
            self,
            move |_| {
                this.imp().tilemap_drawing.queue_draw();
        ...
        state.connect_tile_bpp_notify(clone!(
            ...
            move |state| {
                this.imp()
                    .mode_select
                    .set_sensitive(state.tile_bpp() == Bpp::Two);
            }
        ));

        self.imp().tilemap_drawing.set_draw_func(clone!(
            #[weak(rename_to = this)]
            self,
            #[weak]
            state,
    ...
    fn setup_gesture(&self, state: &Window) {
        ...
        drag_event.connect_drag_begin(clone!(
            #[weak(rename_to = this)]
            self,
            move |_, x, y| {
                let imp = this.imp();
                ...
                let Some(new_idx) = this.cursor_to_idx(x, y) else {
                    return;
                };
        ...
        drag_event.connect_drag_update(clone!(
            #[weak(rename_to = this)]
            self,
            move |drag, dx, dy| {
                let imp = this.imp();
                ...
                // calculate tile index
                let Some((x, y)) = drag.start_point() else {
                    return;
                };
                let Some(new_idx) = this.cursor_to_idx(x + dx, y + dy) else {
                    return;
                };
        ...
        drag_event.connect_drag_end(clone!(
            #[weak(rename_to = this)]
            self,
            #[weak]
            state,
    ...
    fn file_actions(&self, state: &Window) {
        ...
        let action_save = ActionEntry::builder("save")
            .activate(clone!(
                ...
                state,
                move |_, _, _| {
                    let Some(filepath) = state.tilemap_file() else {
                        return;
                    };
